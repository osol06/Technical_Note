...
### LVMとは
LVM(Logical Volume Manager)とは、何台かのディスクをひとつのグループにまとめ、そこから論理ボリューム(パーティション)を切り出すことのできる機能です。LVMでは、システムの運用を継続したまま論理ボリュームのサイズ変更やディスク追加が可能です。
...
```
まずは、パーティションテーブルが未設定のため、msdos(MBR)を設定。  
...
**下記はlvm version1の場合の話でした。現在はlvm version2が主流であり、PEの数に制限はありません。ただし、PEサイズが小さいと処理速度が落ちるため、ちゃんと設計する必要あり**
***   
